...
pyre_c = re.compile(r'^(?P<prefix>#* *)Copyright \(C\) ((?P<start>\d{4})-)?(?P<end>\d{4})')
pyre_n = re.compile(r'^(?P<prefix>#* *)Copyright ((?P<start>\d{4})-)?(?P<end>\d{4})')
new_c = '{}Copyright (C) {}-{} {}'
new_n = '{}Copyright {}-{} {}'
        ...
        try:
            for line in in_file:
                ...
                if int(start) == this_year:
                    out_file.write(line)
                    continue
                print(new.format(prefix, start, this_year, owner), file=out_file)
                print('=>', path)
                for line in in_file:
                    out_file.write(line)
...
def do_walk():
    ...
    for root, dirs, files in os.walk('.'):
        ...
        for file_name in files:
            if os.path.splitext(file_name)[1] in ('.pyc', '.gz'):
                continue
