...
::
    ...
    >>> ignore = cleanups.callback(output_directory.cleanup)
    >>> command('mailman aliases --directory ' + str(output_directory))

For Postfix, there are two files in the output directory.

    >>> files = sorted(f for f in output_directory.listdir() if not str(f).endswith('.created-db'))
    >>> for file in files:
    ...     print(file)
    postfix_domains
    postfix_lmtp
...
The transport map file contains all the aliases for the mailing list.
    ...
    >>> with File(output_directory, 'postfix_lmtp') as fp:
    ...     print(fp.read())
...
The relay domains file contains a list of all the domains.
    ...
    >>> with File(output_directory, 'postfix_domains') as fp:
    ...     print(fp.read())
    # AUTOMATICALLY GENERATED BY MAILMAN ON ...
    ...
